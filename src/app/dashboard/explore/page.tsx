    ...
    {categories.map(cat => (
      <button
        ...
        style={{
          ...
          background: selected === cat ? 'linear-gradient(135deg, #810100, #810100)' : NEU.CARD,
          color: selected === cat ? '#fff' : '#2E2523',
          boxShadow: selected === cat ? '4px 4px 10px rgba(129,1,0,.3), -2px -2px 6px rgba(255,255,255,.5)' : NEU.raised,
  ...
  >
    ...
    <div style={{ position: 'relative', height: 200, padding: 10 }}>
      ...
      <div style={{
        ...
        color: activity.category === 'Adventure' ? '#059669' : activity.category === 'Culture' ? '#7C3AED' : '#810100',
      ...
      <div style={{
        ...
        <span style={{ fontWeight: 800, fontSize: '0.8rem', color: '#1B1716', fontFamily: 'Inter, sans-serif' }}>{activity.rating}</span>
      </div>
    </div>
    ...
    <div style={{ padding: '6px 18px 18px', flex: 1, display: 'flex', flexDirection: 'column', gap: 10 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
        ...
        <span style={{ fontSize: '0.8rem', fontWeight: 700, color: '#6B5E5C', fontFamily: 'Inter, sans-serif' }}>{activity.location}</span>
      </div>

      <h3 style={{ fontFamily: 'Space Grotesk, sans-serif', fontWeight: 700, fontSize: '1.05rem', color: '#1B1716', margin: 0, lineHeight: 1.3, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
        {activity.title}
      </h3>

      <p style={{ fontSize: '0.82rem', color: '#9B8E8C', fontFamily: 'Inter, sans-serif', lineHeight: 1.5, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden', margin: 0 }}>
        {activity.description}
      </p>

      {/* Meta */}
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginTop: 4 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: '0.78rem', color: '#6B5E5C', fontFamily: 'Inter, sans-serif', fontWeight: 500 }}>
          <Clock size={13} color="#9B8E8C" /> {activity.duration}
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: '0.78rem', color: '#6B5E5C', fontFamily: 'Inter, sans-serif', fontWeight: 500 }}>
          <Users size={13} color="#9B8E8C" /> {activity.groupSize}
        </div>
      </div>

      {/* Footer */}
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 'auto', paddingTop: 12, borderTop: '1px solid rgba(139,120,112,.15)' }}>
        <div style={{ fontSize: '0.78rem', color: '#9B8E8C', fontFamily: 'Inter, sans-serif' }}>
          <strong style={{ color: '#2E2523' }}>{activity.reviews.toLocaleString()}</strong> reviews
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <span style={{ fontFamily: 'Space Grotesk, sans-serif', fontWeight: 800, fontSize: '1.15rem', color: '#1B1716' }}>{activity.price}</span>
          <button
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 36, maxWidth: 1200, margin: '0 auto', paddingBottom: 48 }}>
      ...
      <div style={{ background: NEU.CARD, borderRadius: 32, boxShadow: NEU.raised, padding: '1.5rem 2rem', border: 'none', display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: 16 }}>
        ...
        <div style={{ position: 'relative', width: '100%', maxWidth: 320 }}>
          <Search size={16} style={{ position: 'absolute', left: 16, top: '50%', transform: 'translateY(-50%)', color: '#810100', pointerEvents: 'none' }} />
          <input
            ...
            style={{
              ...
              fontSize: '0.9rem', color: '#1B1716', fontFamily: 'Inter, sans-serif',
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 8px' }}>
        <p style={{ fontSize: '0.9rem', fontWeight: 500, color: '#6B5E5C', fontFamily: 'Inter, sans-serif' }}>
          Showing <strong style={{ color: '#1B1716' }}>{filtered.length}</strong> activities
          {filter !== 'All' && <> in <strong style={{ color: '#810100' }}>{filter}</strong></>}
          {search && <> matching "<strong style={{ color: '#1B1716' }}>{search}</strong>"</>}
        </p>
      </div>
      ...
      {filtered.length > 0 ? (
        ...
        <div style={{
          ...
          <h3 style={{ fontFamily: 'Space Grotesk, sans-serif', fontWeight: 700, fontSize: '1.2rem', color: '#1B1716', margin: '0 0 8px' }}>No activities found</h3>
          <p style={{ color: '#9B8E8C', fontSize: '0.9rem', marginBottom: 20, fontFamily: 'Inter, sans-serif' }}>Try adjusting your search or filters.</p>
          <button className="neu-btn" onClick={() => { setSearch(''); setFilter('All') }} style={{ padding: '10px 22px', color: '#2E2523' }}>
            Clear Filters
          </button>
        </div>
      )}
      ...
      <div style={{ background: NEU.CARD, borderRadius: 32, boxShadow: NEU.raised, padding: '2.5rem 2.5rem', textAlign: 'center', border: 'none' }}>
        ...
        <h3 style={{ fontFamily: 'Space Grotesk, sans-serif', fontWeight: 700, fontSize: '1.15rem', color: '#1B1716', margin: '0 0 8px' }}>
          Want personalized recommendations?
        </h3>
        <p style={{ color: '#6B5E5C', fontSize: '0.9rem', maxWidth: 440, margin: '0 auto 20px', fontFamily: 'Inter, sans-serif' }}>
