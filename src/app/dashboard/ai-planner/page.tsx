          ...
          <React.Fragment key={label}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <div style={{
                ...
                background: isActive ? 'linear-gradient(135deg, #6C63FF, #8B5CF6)' : NEU.PAGE,
                color: isActive ? '#fff' : isComplete ? '#6C63FF' : '#94A3B8',
                boxShadow: isActive ? '4px 4px 10px rgba(108,99,255,.35), -2px -2px 6px rgba(255,255,255,.5)' : NEU.pressed,
                transition: 'all .3s ease',
              }}>
                {isComplete ? <Check size={14} /> : idx}
              </div>
              <span style={{ fontFamily: 'Inter, sans-serif', fontWeight: isActive ? 700 : 500, fontSize: '0.85rem', color: isActive ? '#1E293B' : isComplete ? '#6C63FF' : '#94A3B8' }}>{label}</span>
            </div>
...
const VibeButton = ({ vibe, selected, onSelect }: { vibe: typeof VIBES[0]; selected: boolean; onSelect: () => void }) => {
  ...
  return (
    ...
    >
      <div style={{ width: 44, height: 44, borderRadius: 14, display: 'flex', alignItems: 'center', justifyContent: 'center', background: NEU.PAGE, boxShadow: selected ? NEU.raised : NEU.pressed, color: selected ? '#6C63FF' : '#94A3B8', transition: 'all .3s ease' }}>
        <Icon size={22} />
      </div>
      <span style={{ fontWeight: 700, fontSize: '0.8rem', color: selected ? '#6C63FF' : '#334155', lineHeight: 1.2 }}>{vibe.label}</span>
    </button>
  )
}
...
const CompanionButton = ({ item, selected, onSelect }: { item: typeof COMPANIONS[0]; selected: boolean; onSelect: () => void }) => {
  ...
  return (
    <button type="button" onClick={onSelect} style={{
      ...
      onMouseLeave={e => { if (!selected) { (e.currentTarget as HTMLElement).style.transform = 'scale(1)'; (e.currentTarget as HTMLElement).style.boxShadow = NEU.raised } }}
    >
      <div style={{ width: 48, height: 48, borderRadius: 16, display: 'flex', alignItems: 'center', justifyContent: 'center', background: NEU.PAGE, boxShadow: selected ? NEU.raised : NEU.pressed, color: selected ? '#6C63FF' : '#94A3B8', transition: 'all .3s ease' }}>
        <Icon size={24} />
      </div>
      <span style={{ fontWeight: 700, fontSize: '0.85rem', color: selected ? '#6C63FF' : '#334155' }}>{item.label}</span>
    </button>
  )
}
...
const NeuInput = ({ placeholder, value, onChange, type = 'text', icon: Icon, prefix, style: extraStyle = {}, ...rest }: any) => (
  <div style={{ position: 'relative', ...extraStyle }}>
    ...
    <input
      ...
      style={{ width: '100%', paddingLeft: (Icon || prefix) ? 48 : 18, paddingRight: 18, paddingTop: 16, paddingBottom: 16, background: NEU.PAGE, borderRadius: 16, border: 'none', outline: 'none', fontSize: '1rem', color: '#1E293B', fontFamily: 'Inter, sans-serif', boxShadow: NEU.input, transition: 'box-shadow .25s ease' }}
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 32, maxWidth: 900, margin: '0 auto', paddingBottom: 48 }}>
      <section style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 20, paddingTop: 8 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <span style={{ fontFamily: 'Space Grotesk, sans-serif', fontWeight: 800, fontSize: '1.5rem', color: '#1E293B' }}>Smart Journey Builder</span>
        </div>
        <Stepper current={1} />
      </section>
      ...
      <NeuCard>
        <h2 style={{ fontFamily: 'Space Grotesk, sans-serif', fontWeight: 700, fontSize: '1.25rem', color: '#1E293B', display: 'flex', alignItems: 'center', gap: 10, margin: '0 0 6px' }}>
          <div style={{ width: 36, height: 36, borderRadius: 12, background: NEU.PAGE, boxShadow: NEU.pressed, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><MapPin size={18} color="#6C63FF" /></div>
          Where would you like to go?
        </h2>
        <p style={{ color: '#94A3B8', fontSize: '0.9rem', fontFamily: 'Inter, sans-serif', marginBottom: 28 }}>Search for a city, country, or landmark — autocompleted via Google Places.</p>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: 4 }}>
              {['5000', '15000', '25000', '50000'].map(val => (
                <button key={val} type="button" onClick={() => setBudget(val)} style={{ padding: '6px 14px', borderRadius: 12, border: 'none', cursor: 'pointer', fontFamily: 'Inter, sans-serif', fontWeight: 700, fontSize: '0.78rem', background: NEU.PAGE, transition: 'all .25s ease', boxShadow: budget === val ? NEU.pressed : NEU.raised, color: budget === val ? '#6C63FF' : '#64748B', transform: budget === val ? 'scale(.97)' : 'scale(1)' }}>
      ...
      <NeuCard>
        <h2 style={{ fontFamily: 'Space Grotesk, sans-serif', fontWeight: 700, fontSize: '1.25rem', color: '#1E293B', display: 'flex', alignItems: 'center', gap: 10, margin: '0 0 6px' }}>
          <div style={{ width: 36, height: 36, borderRadius: 12, background: NEU.PAGE, boxShadow: NEU.pressed, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Sparkles size={18} color="#6C63FF" /></div>
          What's your travel vibe?
        </h2>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(110px, 1fr))', gap: 14, marginTop: 24 }}>
      ...
      <NeuCard>
        <h2 style={{ fontFamily: 'Space Grotesk, sans-serif', fontWeight: 700, fontSize: '1.25rem', color: '#1E293B', display: 'flex', alignItems: 'center', gap: 10, margin: '0 0 6px' }}>
          <div style={{ width: 36, height: 36, borderRadius: 12, background: NEU.PAGE, boxShadow: NEU.pressed, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Users size={18} color="#6C63FF" /></div>
      ...
      <div>
        <button className="neu-btn-primary" onClick={handleGenerate} disabled={!destination || !numberOfDays || !budget || !selectedVibe || !companion} style={{ width: '100%', padding: '1.25rem', fontSize: '1.05rem', gap: 12 }}>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 28, maxWidth: 1100, margin: '0 auto', paddingBottom: 48, position: 'relative' }}>
      ...
      <div style={{
        ...
        background: '#E8EDF4', boxShadow: '-10px 0 30px rgba(163,177,198,.3)',
        ...
        {selectedActivity && (
          <>
            <div style={{ padding: '24px 24px 16px', display: 'flex', justifyContent: 'flex-end' }}>
              <button onClick={() => setSelectedActivity(null)} style={{ background: NEU.PAGE, border: 'none', width: 40, height: 40, borderRadius: '50%', boxShadow: NEU.raised, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: '#64748B' }}>
                <X size={20} />
              </button>
            </div>
            ...
            <div className="custom-scrollbar" style={{ flex: 1, overflowY: 'auto', padding: '0 24px 40px', display: 'flex', flexDirection: 'column', gap: 24 }}>
              ...
              <div>
                <h2 style={{ fontFamily: 'Space Grotesk, sans-serif', fontWeight: 800, fontSize: '1.6rem', color: '#1E293B', margin: '0 0 12px', lineHeight: 1.2 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 20 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: NEU.CARD, padding: '12px', borderRadius: 16, boxShadow: NEU.raised }}>
                    ...
                    <div>
                      <div style={{ fontSize: 10, color: '#94A3B8', fontWeight: 700, textTransform: 'uppercase' }}>Rating</div>
                      <div style={{ fontSize: '0.9rem', color: '#1E293B', fontWeight: 700 }}>{selectedActivity.rating || '4.5'}</div>
                    </div>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: NEU.CARD, padding: '12px', borderRadius: 16, boxShadow: NEU.raised }}>
                    <MapPin size={16} color="#3B82F6" />
                    <div>
                      <div style={{ fontSize: 10, color: '#94A3B8', fontWeight: 700, textTransform: 'uppercase' }}>Location</div>
                      <div style={{ fontSize: '0.9rem', color: '#1E293B', fontWeight: 700, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: 100 }}>{selectedActivity.city}</div>
                    </div>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: NEU.CARD, padding: '12px', borderRadius: 16, boxShadow: NEU.raised }}>
                    <Clock size={16} color="#22C55E" />
                    <div>
                      <div style={{ fontSize: 10, color: '#94A3B8', fontWeight: 700, textTransform: 'uppercase' }}>Time</div>
                      <div style={{ fontSize: '0.85rem', color: '#1E293B', fontWeight: 700 }}>{selectedActivity.time?.split('-')[0]?.trim() || 'Schedule'}</div>
                    </div>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: NEU.CARD, padding: '12px', borderRadius: 16, boxShadow: NEU.raised }}>
                    <DollarSign size={16} color="#EF4444" />
                    <div>
                      <div style={{ fontSize: 10, color: '#94A3B8', fontWeight: 700, textTransform: 'uppercase' }}>Expense</div>
                      <div style={{ fontSize: '0.9rem', color: '#1E293B', fontWeight: 700 }}>{selectedActivity.expense > 0 ? `${currencySymbol}${selectedActivity.expense}` : 'Free'}</div>
                    </div>
                  </div>
                </div>
                ...
                <div style={{ display: 'flex', gap: 12, marginBottom: 24 }}>
                  <div style={{ flex: 1, background: NEU.PAGE, borderRadius: 16, padding: '12px', boxShadow: NEU.pressed, textAlign: 'center' }}>
                    ...
                    <div style={{ fontSize: '0.85rem', fontWeight: 700, color: '#1E293B' }}>Day 1</div>
                    <div style={{ fontSize: '0.8rem', color: '#64748B' }}>32°C</div>
                  </div>
                  <div style={{ flex: 1, background: NEU.PAGE, borderRadius: 16, padding: '12px', boxShadow: NEU.pressed, textAlign: 'center' }}>
                    <CloudSun size={20} color="#3B82F6" style={{ margin: '0 auto 6px' }} />
                    <div style={{ fontSize: '0.85rem', fontWeight: 700, color: '#1E293B' }}>Day 2</div>
                    <div style={{ fontSize: '0.8rem', color: '#64748B' }}>30°C</div>
                  </div>
                  <div style={{ flex: 1, background: NEU.PAGE, borderRadius: 16, padding: '12px', boxShadow: NEU.pressed, textAlign: 'center' }}>
                    <CloudRain size={20} color="#8B5CF6" style={{ margin: '0 auto 6px' }} />
                    <div style={{ fontSize: '0.85rem', fontWeight: 700, color: '#1E293B' }}>Day 3</div>
                    <div style={{ fontSize: '0.8rem', color: '#64748B' }}>28°C</div>
                  </div>
                </div>

                {/* Actions */}
                <h3 style={{ fontFamily: 'Inter, sans-serif', fontWeight: 700, fontSize: '1.05rem', color: '#1E293B', marginBottom: 12 }}>Actions</h3>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  <button onClick={() => { setMapMode('single'); setFocusedActivities([selectedActivity]) }} style={{ width: '100%', padding: '14px', borderRadius: 16, background: '#F8FAFC', border: 'none', boxShadow: NEU.raised, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, fontWeight: 700, color: '#6C63FF', cursor: 'pointer' }}>
                  ...
                  <div style={{ display: 'flex', gap: 12 }}>
                    ...
                    }} style={{ flex: 1, padding: '14px', borderRadius: 16, background: '#F8FAFC', border: 'none', boxShadow: NEU.raised, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, fontWeight: 700, color: '#334155', cursor: 'pointer' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16 }}>
        <button onClick={() => { setStep(1); setLoading(false) }} className="neu-btn" style={{ padding: '10px 20px', color: '#334155', fontSize: '0.875rem' }}>
      ...
      {loading && (
        <NeuCard style={{ minHeight: 360, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', gap: 24 }}>
          ...
          <div><h3 style={{ fontFamily: 'Space Grotesk, sans-serif', fontWeight: 700, fontSize: '1.4rem', color: '#1E293B', margin: '0 0 10px' }}>Crafting your perfect itinerary...</h3></div>
      ...
      {!loading && itinerary && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 32 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8, padding: '0 1rem' }}>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '5px 14px', borderRadius: 999, background: NEU.PAGE, boxShadow: NEU.raised, fontSize: 10, fontWeight: 800, letterSpacing: '0.14em', textTransform: 'uppercase', color: '#6C63FF', fontFamily: 'Inter, sans-serif', alignSelf: 'flex-start' }}>
              <Navigation size={12} /> AI Generated Plan
            </div>
            <h1 style={{ fontFamily: 'Space Grotesk, sans-serif', fontWeight: 800, fontSize: '2.5rem', color: '#1E293B', margin: 0, lineHeight: 1.1 }}>{itinerary.destination}</h1>
            <p style={{ fontSize: '1rem', color: '#64748B', fontFamily: 'Inter, sans-serif' }}>{itinerary.days} Days • {itinerary.vibe} • {itinerary.companion}</p>
          </div>
              ...
              {[
                ...
                <div key={item.label} style={{ background: NEU.CARD, borderRadius: 24, boxShadow: NEU.raised, border: 'none', borderLeft: `4px solid ${item.accent}`, padding: '1.25rem 1.5rem', flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                    <div style={{ fontSize: 10, fontWeight: 800, color: '#94A3B8', textTransform: 'uppercase', letterSpacing: '0.08em', fontFamily: 'Inter, sans-serif' }}>{item.label}</div>
                    <item.icon size={14} color={item.accent} />
                  </div>
                  <div style={{ fontFamily: 'Space Grotesk, sans-serif', fontWeight: 800, fontSize: '1.25rem', color: '#1E293B', marginBottom: 2 }}>{item.value}</div>
                  <div style={{ fontSize: 10, color: '#94A3B8', fontFamily: 'Inter, sans-serif' }}>{item.sub}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div>
            <h3 style={{ fontFamily: 'Space Grotesk, sans-serif', fontWeight: 700, fontSize: '1.25rem', color: '#1E293B', display: 'flex', alignItems: 'center', gap: 8, marginBottom: 20 }}>
              <Briefcase size={20} color="#6C63FF" /> Day-by-Day Plan
            </h3>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
              {itinerary.dailyItinerary.map((day: any) => (
                <NeuCard key={day.day}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16, paddingBottom: 16, borderBottom: '1px solid rgba(163,177,198,.15)' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      <div style={{ width: 40, height: 40, borderRadius: 14, background: NEU.PAGE, boxShadow: NEU.pressed, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Space Grotesk, sans-serif', fontWeight: 800, color: '#6C63FF' }}>
                        {day.day}
                      </div>
                      <div>
                        <span style={{ fontFamily: 'Space Grotesk, sans-serif', fontWeight: 700, fontSize: '1.1rem', color: '#1E293B', display: 'block' }}>Day {day.day} Schedule</span>
                        {day.area && <span style={{ fontSize: '0.8rem', color: '#64748B', fontFamily: 'Inter, sans-serif' }}>{day.area}</span>}
                      </div>
                    </div>
                    <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
                      <span style={{ fontSize: '0.85rem', fontFamily: 'Inter, sans-serif', color: '#94A3B8' }}>
                        Cost: <strong style={{ color: '#6C63FF' }}>{currencySymbol}{day.totalSpent}</strong>
                      </span>
                      <button onClick={() => handleDaySelect(day.activities)} style={{ background: NEU.PAGE, border: 'none', borderRadius: 12, padding: '8px 12px', fontSize: '0.8rem', fontWeight: 700, color: '#6C63FF', boxShadow: NEU.raised, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}>
                    ...
                    {day.activities.map((act: any, i: number) => {
                      if (act.isMeal) return (
                        <button key={i} onClick={() => handleActivitySelect(act)} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 18px', borderRadius: 16, background: NEU.PAGE, border: 'none', boxShadow: NEU.pressed, cursor: 'pointer', width: '100%', textAlign: 'left', transition: 'transform 0.2s' }} onMouseEnter={e => (e.currentTarget.style.transform = 'translateY(-1px)')} onMouseLeave={e => (e.currentTarget.style.transform = 'translateY(0)')}>
                          ...
                          <div style={{ flex: 1 }}>
                            <span style={{ fontFamily: 'Inter, sans-serif', fontWeight: 600, fontSize: '0.9rem', color: '#334155' }}>{act.name}</span>
                            <span style={{ display: 'block', fontSize: '0.75rem', color: '#94A3B8', marginTop: 2 }}>{act.time}</span>
                          </div>
                        ...
                        >
                          <div style={{ flex: 1 }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
                              ...
                              <span style={{ fontSize: '0.75rem', fontWeight: 700, color: '#94A3B8', fontFamily: 'Inter, sans-serif' }}>{act.time}</span>
                            </div>
                            <h4 style={{ fontFamily: 'Space Grotesk, sans-serif', fontWeight: 700, color: '#1E293B', margin: '0 0 4px', fontSize: '1rem' }}>{act.name}</h4>
                          </div>
              ...
              {itinerary.smartRecommendations.map((rec: any, i: number) => (
                ...
                >
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                      <Star size={13} color="#F59E0B" style={{ fill: '#F59E0B' }} />
                      <span style={{ fontWeight: 700, fontSize: '0.85rem', color: '#1E293B', fontFamily: 'Inter, sans-serif' }}>{rec.rating}</span>
                    </div>
                  </div>
                  <h5 style={{ fontFamily: 'Space Grotesk, sans-serif', fontWeight: 700, color: '#1E293B', margin: '0 0 8px', fontSize: '1rem' }}>{rec.name}</h5>
                  <p style={{ fontSize: '0.85rem', color: '#94A3B8', fontFamily: 'Inter, sans-serif', lineHeight: 1.6, margin: 0 }}>{rec.desc}</p>
                </div>
              ))}
            </div>
          </NeuCard>
