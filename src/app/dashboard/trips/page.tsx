...
const TabButton = ({ active, onClick, children, icon: Icon }: any) => (
  <button onClick={onClick} style={{
    ...
    background: active ? NEU.PAGE : NEU.CARD, boxShadow: active ? NEU.pressed : NEU.raised, color: active ? '#6C63FF' : '#64748B', transition: 'all .3s ease', transform: active ? 'scale(.97)' : 'scale(1)',
  }}>
  ...
  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, padding: '14px 0', borderBottom: '1px solid rgba(163,177,198,.15)' }}>
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
      ...
      <input type="number" value={value} onChange={e => onChange(parseFloat(e.target.value) || 0)}
        style={{ width: 120, padding: '8px 14px', borderRadius: 12, border: 'none', outline: 'none', background: NEU.PAGE, boxShadow: NEU.input, fontWeight: 700, fontSize: '0.9rem', color: '#1E293B', fontFamily: 'Inter, sans-serif', transition: 'box-shadow .25s ease' }}
        onFocus={e => { e.currentTarget.style.boxShadow = `${NEU.input}, 0 0 0 2px rgba(108,99,255,.2)` }} onBlur={e => { e.currentTarget.style.boxShadow = NEU.input }} />
    </div>
  </div>
)
...
const NoteItem = ({ note, onDelete }: any) => (
  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 14, padding: '16px 18px', borderRadius: 20, background: NEU.CARD, boxShadow: NEU.raised, border: 'none' }}>
    ...
    <button onClick={() => onDelete(note.id)} style={{ padding: 8, borderRadius: 12, border: 'none', cursor: 'pointer', background: NEU.CARD, boxShadow: NEU.raised, color: '#94A3B8', transition: 'color .2s ease', flexShrink: 0 }}
      onMouseEnter={e => { (e.currentTarget as HTMLElement).style.color = '#EF4444' }} onMouseLeave={e => { (e.currentTarget as HTMLElement).style.color = '#94A3B8' }}><Trash2 size={14} /></button>
  </div>
)
  ...
  return (
    <div onClick={onClick} style={{ background: NEU.CARD, borderRadius: 28, boxShadow: NEU.raised, border: 'none', overflow: 'hidden', cursor: 'pointer', transition: 'all .3s ease' }}
      ...
      <div style={{ position: 'relative', height: 160, padding: 10 }}>
        ...
        <button onClick={onDelete} style={{ position: 'absolute', top: 20, right: 20, width: 36, height: 36, borderRadius: 12, border: 'none', cursor: 'pointer', background: 'rgba(248,250,252,.8)', backdropFilter: 'blur(8px)', boxShadow: '4px 4px 10px rgba(163,177,198,.2), -4px -4px 10px rgba(255,255,255,.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#94A3B8', transition: 'color .2s ease', zIndex: 10 }}
          onMouseEnter={e => { (e.currentTarget as HTMLElement).style.color = '#EF4444' }} onMouseLeave={e => { (e.currentTarget as HTMLElement).style.color = '#94A3B8' }}><Trash2 size={14} /></button>
      </div>
      <div style={{ padding: '6px 18px 18px' }}>
        ...
        <p style={{ fontSize: '0.82rem', color: '#94A3B8', fontFamily: 'Inter, sans-serif', lineHeight: 1.5, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden', margin: '6px 0 12px' }}>{trip.description || 'No description yet.'}</p>
        <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 14, paddingTop: 12, borderTop: '1px solid rgba(163,177,198,.15)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: '0.78rem', color: '#64748B', fontFamily: 'Inter, sans-serif' }}><Calendar size={13} color="#6C63FF" /> {startF} – {endF}</div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: '0.78rem', color: '#64748B', fontFamily: 'Inter, sans-serif' }}><MapPin size={13} color="#6C63FF" /> <span style={{ maxWidth: 130, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{dest}</span></div>
          {trip.totalBudget && <div style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: '0.78rem', fontWeight: 700, color: '#1E293B', fontFamily: 'Inter, sans-serif' }}><DollarSign size={13} color="#22C55E" /> {currency}{trip.totalBudget.toLocaleString()}</div>}
        </div>
      </div>
    </div>
  )
              ...
              <div className="custom-scrollbar" style={{ flex: 1, overflowY: 'auto', padding: '0 24px 40px', display: 'flex', flexDirection: 'column', gap: 24 }}>
                ...
                <div>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                    <button onClick={() => { setMapMode('single'); setFocusedActivities([selectedActivity]); setActiveTab('itinerary') }} style={{ width: '100%', padding: '14px', borderRadius: 16, background: '#F8FAFC', border: 'none', boxShadow: NEU.raised, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, fontWeight: 700, color: '#6C63FF', cursor: 'pointer' }}>
                ...
                <NeuCard key={day.day}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16, paddingBottom: 14, borderBottom: '1px solid rgba(163,177,198,.15)' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      <div style={{ width: 40, height: 40, borderRadius: 14, background: NEU.PAGE, boxShadow: NEU.pressed, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Space Grotesk, sans-serif', fontWeight: 800, color: '#6C63FF' }}>{day.day}</div>
                      <div>
                    ...
                    <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
                      <span style={{ fontSize: '0.85rem', fontFamily: 'Inter, sans-serif', color: '#94A3B8' }}>Cost: <strong style={{ color: '#6C63FF' }}>{currencySymbol}{day.totalSpent}</strong></span>
                    ...
                    {day.activities.map((act: any, i: number) => {
                      if (act.isMeal) return (
                        <button key={i} onClick={() => handleActivitySelect(act)} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 18px', borderRadius: 16, background: NEU.PAGE, border: 'none', boxShadow: NEU.pressed, cursor: 'pointer', width: '100%', textAlign: 'left', transition: 'transform 0.2s' }} onMouseEnter={e => (e.currentTarget.style.transform = 'translateY(-1px)')} onMouseLeave={e => (e.currentTarget.style.transform = 'translateY(0)')}>
                          ...
                          <div style={{ flex: 1 }}><span style={{ fontFamily: 'Inter, sans-serif', fontWeight: 600, fontSize: '0.9rem', color: '#334155' }}>{act.name}</span><span style={{ display: 'block', fontSize: '0.75rem', color: '#94A3B8', marginTop: 2 }}>{act.time}</span></div>
                      ...
                      return (
                        <button key={i} onClick={() => handleActivitySelect(act)} style={{ display: 'flex', alignItems: 'flex-start', gap: 16, padding: '14px 20px', borderRadius: 20, background: NEU.CARD, border: 'none', boxShadow: NEU.raised, transition: 'all .25s ease', cursor: 'pointer', width: '100%', textAlign: 'left' }}
                          ...
                          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 8, flexShrink: 0 }}>
                            {act.rating && <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}><Star size={13} color="#F59E0B" style={{ fill: '#F59E0B' }} /><span style={{ fontWeight: 700, fontSize: '0.8rem', color: '#1E293B', fontFamily: 'Inter, sans-serif' }}>{act.rating}</span></div>}
                            <span style={{ fontSize: '0.8rem', fontWeight: 700, color: '#6C63FF', fontFamily: 'Inter, sans-serif' }}>{act.expense > 0 ? `${currencySymbol}${act.expense}` : 'Free'}</span>
                          </div>
                        </button>
                      )
                    })}
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <input
                ...
                style={{
                  ...
                  background: NEU.PAGE, boxShadow: NEU.input, fontFamily: 'Inter, sans-serif', fontSize: '0.9rem', color: '#1E293B',
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 28, maxWidth: 1200, margin: '0 auto', paddingBottom: 48 }}>
      <section style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: 16 }}>
        <div>
          <h1 style={{ fontFamily: 'Space Grotesk, sans-serif', fontWeight: 800, fontSize: 'clamp(1.6rem, 3vw, 2.2rem)', color: '#1E293B', margin: 0, display: 'flex', alignItems: 'center', gap: 12 }}>
